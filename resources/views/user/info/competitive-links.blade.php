@php
    $page_title = 'View Competitive Exam';
@endphp
...
@push('css')
    <style>
        ...
        .exam-list {
            ...
            height: 300px;
            padding-top: 50px;
            /* Adjust this height to match the header height */
        }
        ...
        .exam-list .row {
            ...
            border-bottom: 1px solid #ccc;
            z-index: 1;
            /* Ensure rows are below the header */
        }
...
@section('main_section')
    <main>
        <div class="container">
            ...
            <h2 style="text-align:center; margin-bottom: 25px;">Latest Competitive Exams</h2>
            <div class="exam-list" id="exam-list">
                <div class="header">
                    <div class="column">Exam Link</div>
                ...
                <div class="rows">
                    @foreach($exams as $exam)
                                    ...
                                    @foreach($examUrls as $url)
                                        <div class="row">
                                            <div class="column"><a href="/competitive-exam/{{ $exam->id }}"><strong>{{ $url }}</strong></a>
                                            </div>
                                            <div class="column">{{ \Carbon\Carbon::parse($exam->exam_opening_date)->format('d-M-Y h:i A') }}
                                            </div>
                                            <div class="column">{{ \Carbon\Carbon::parse($exam->exam_closing_date)->format('d-M-Y h:i A') }}
                                            </div>
                                        </div>
                                    @endforeach
                    @endforeach
